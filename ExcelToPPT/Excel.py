...
class Pyxlchart(object):
    ...
    def _get_Charts_In_Worksheet(self,wb,worksheet = "", chartname = ""):
        ...
        for cht in sht.ChartObjects():
            try:
                cht_title = cht.chart.ChartTitle.text
            except:
    ...
    def _get_filename(self,chartname):
        ...
        if self.ImageFilename != "":
            return self.ImageFilename + "." + self.ImageType
        else:
            return chartname + '.' + self.ImageType
if __name__ == "__main__":
    xl = Pyxlchart()
    xl.WorkbookDirectory = r"G:\youyu\ExcelToPPT\excel"+'\\'
    xl.WorkbookFilename = "广平项目进度报告-2018年第9周 （2018.03.01).xlsx"
    xl.SheetName = "广平项目周进度表-2018年第9周"
    xl.ImageFilename = "广平项目"
    xl.ExportPath = r"G:\youyu\ExcelToPPT\excel"
    xl.ChartName = "123"
    xl.start_export()
