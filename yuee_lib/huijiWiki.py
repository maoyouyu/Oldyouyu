...
class HuijiWiki(threading_danteng.ObjectDanteng):
    ...
    _api_url = None
    _index_url = None
    _user_agent = {'user-agent': 'Yuee-Bot/0.1'}
    _edit_token = None
    _cookies = None
    _summary = 'by Yuee_Bot'
    _project = None
    _permission_level = 1
    _permission_high_level_list = ['Yuee bot', 'Yuee_bot']
    _sleep_time = 1
    _timeout = 30
    ...
    def _get_edit_token_nsite(self):
        params = {'action': 'query',
                  'meta': 'tokens'}
        self._que_in.put({'action': 'get_edit_token',
                          'type': 'params',
                          'params': params,
                          'update_progress': False})
        temp_api_url = 'http://%s.joyme.com/api.php' % self._key
        t = WikiPoster(self._que_in, self._que_out, temp_api_url, self._cookies, self._user_agent)
        t.start()
        t.join()
        result = t.get_result()
        try:
            ...
            if edit_token != '+\\':
                self._edit_token = edit_token
                # 数字站
                self._url = 'http://%s.joyme.com/' % self._key
                self._wiki_url = self._url
                self._api_url = temp_api_url
                self._index_url = self._wiki_url + "index.php"
                self._nsite = True
                return True
        except Exception as e:
            self._error_log('获得编辑令牌时发生错误：%s' % e)
        return False
    ...
    # 移动页面
    def move(self, title, new_title, summary='', movetalk=False, movesubpages=False, noredirect=False):
        if summary == '':
            summary = self._summary
        params = {
            'action': (None, 'move'),
    ...
    # 获得一个页面的rawtext
    def raw(self, title, notice=True, filepath='', wikitextpath=''):
...
class WikiPoster(threading_danteng.ThreadDanteng):
    ...
    def _call(self, args, retry_count=0):
        ...
        if response.status_code in [502, 503, 504]:
            ...
            if 'error' in self.result:
                ...
                self._error_code(self.result['error'], args)
                return False
            else:
                return True
        else:
    ...
    # 根据title获得raw_text
    def raw(self, args):
        url = self.api_url.replace('api.php', 'index.php?title=%s&action=raw' % quote(args['title']))
        ...
        if result.status_code == 200:
            ...
            if 'filepath' in args and args['filepath'] != '':
                self.save_file(args['filepath'], result.text)
